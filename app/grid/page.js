          ...
          <div className="pt-[100%] group relative">
            ...
            <div className="absolute inset-0 flex flex-col items-start justify-between p-6 gap-4">
              ...
              <div className="z-50 flex flex-row justify-between">
                ...
                transition-all duration-300 leading-5 w-1/2 mr-2">
                {subheader}
                </h3>

                {/* Role */}
                <h3 className="z-50 text-white tracking-tight text-base opacity-0 group-hover:opacity-100
            ...
            group-hover:bg-white group-hover:text-black group-hover:scale-150 group-hover:m-2 group-hover:px-0.5
...
const filteredVideos = videoData.filter((video) => {
  ...
  return (
    <>
      <div className="px-6 mt-10 text-sm max-w-9xl mx-auto font-[family-name:var(--font-geist-sans)]">
        {/* Entire Page column setup */}
        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 2xl:grid-cols-9">

          {/* Top Navbar */}
          <div className="col-span-full fixed top-2 text-xl z-50">

              <motion.div
              className="flex flex-row min-w-screen justify-start gap-10"
              initial="hidden"
              animate="show"
              variants={animateIn}
              transition={{ duration: 0.5 }}> 
                ...
                {/* SideNav Button */}
                <button 
                className="text-foreground border-1 p-1 px-1.5 pt-1.5 rounded-full border-foreground flex items-center 
                justify-center hover:bg-foreground hover:text-background transition-all duration-200 -ml-1 hover:scale-90"
                ...
                onClick={toggleNav}>
                  ...
                  <svg viewBox="0 0 22 22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="w-5 h-5">
                    <path d={`${showNav ? 'M6 10h10' : 'M2 10h18'}`}
                    className='transition-all duration-200'/>
                    <path d={`${showNav ? 'M6 15h10' : 'M2 15h18'}`}
                    className='transition-all duration-300'/>
                    <path d={`${showNav ? 'M6 5h10' : 'M2 5h18'}`}
                ...
                {/* Resume Button */}
                <motion.button
                  className={`hover:text-foreground text-base font-medium tracking-tight rounded-full px-3 border-1 hover:border-black dark:hover:border-white
                    transition-colors duration-200 z-50 mix-blend-difference
                    ${selectedWork.includes('resume') 
                      ? 'border-black dark:border-white text-foreground' 
                      : 'border-black/0 text-[rgba(0,0,0,0.3)] dark:text-[rgba(255,255,255,0.5)] dark:hover:text-foreground'
                    }`}
          ...
          {/* Navbar Background */}
          <div className="fixed backdrop-blur-md z-40 left-0 top-0 w-full h-12 shadow-[0px_0px_15px_-8px_rgba(0,0,0,0.2)]"></div>
          ...
          <motion.div
            ...
            {showNav && (
              <>
                {/* Reset Button */}
                <motion.div 
                  className="sticky top-16 mt-8 z-50">
                  {showReset && (
                    <motion.button
                      className="group hover:text-background font-medium flex gap-1.5
                      -ml-1 mb-6 pt-1 pb-1 pr-2 pl-2.5 z-50 backdrop-blur-sm
          ...
          > 
            {/* Grid / Page */}
            <motion.div 
            className="grid grid-cols-2 sm:grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 col-span-full gap-1.5 mt-14 md:mt-4">
              <AnimatePresence>
