...
import { websiteIntro } from '../content/website/intro';
import { pageLoad, pageLoadItem } from '../constants/animations';

const WebsiteV2 = ({className, showNav}) => {
  ...
  useEffect(() => {
    ...
    websiteIntro.sections.forEach((section, index) => {
    ...
    >
      ...
      <div className="col-span-2 sticky top-28 h-screen flex flex-col">
        ...
        <motion.h2 
          ...
          variants={pageLoadItem}
        >
            {`This here will be the entire documentation of my process into building a portfolio website from the ground up using a combination of React and Next.js. 

            (Updated for June 2025) 

            If you would like to skip the entire part where I built Portfolio Website v1 (2024) from scratch and are interested in the iteration upon it for Portfolio Website v2 (2025) with exploration and incorporation of 'vibe coding' and rapid prototyping into my process, jump straight to`}
        </motion.h2>
        ...
        <motion.div 
          ...
          variants={pageLoadItem}
        >
          {websiteIntro.sections.map((section, index) => (
            <motion.button 
              key={index}
